...
import { fetchWeather, getWeatherIcon } from '../src/weather';
beforeEach(() => {
...
test('shows weather info after clicking fetch', async () => {
  ...
  getWeatherIcon.mockReturnValue('assets/clear.png');
  ...
  await screen.findByText(/Temperature: 25°C/i);

  expect(fetchWeather).toHaveBeenCalledWith('Addis Ababa');
  expect(getWeatherIcon).toHaveBeenCalledWith('Clear');
  ...
  expect(screen.getByAltText('Weather Icon')).toBeVisible();
});
